# MindLoom Quick Save Extension

One-click save for the current tab URL into your MindLoom shelf.

## What it does
- Click the extension icon while browsing.
- It sends the current tab URL to your existing MindLoom backend (`POST /api/links`).
...
4. Select this folder: `browser-extension/mindloom-quick-save`

## Configure once
1. In extension details, open **Extension options**
2. Set:
   - `API Base URL` (default `http://localhost:5000`)
   - `JWT Token` (`mindloom_token` from your web app localStorage)
...
- `MindLoom > Save current URL to shelf` to save to a specific shelf one time
- `MindLoom > Set default shelf` to pick the default shelf for normal icon-click saves
- `MindLoom > Save current URL (default shelf)` for quick save with your selected default

## How to get token
On your MindLoom app tab, open DevTools Console and run:

```js
localStorage.getItem('mindloom_token')
```
